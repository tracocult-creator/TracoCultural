...
const VerificarCodigo = () => {
  ...
  const origem = location.state?.origem || null
  ...
  const infoInicial = origem === 'login' ? 'Confirme seu email para poder entrar.' : ''

  const inputsRef = useRef([])

  // Se veio de um login bloqueado (email não verificado), o código do cadastro
  // pode já ter expirado — reenvia um novo automaticamente ao abrir a tela.
  useEffect(() => {
    if (origem === 'login' && emailInicial) {
      reenviarCodigo(emailInicial).catch(() => {
        // Se falhar (ex.: já verificado nesse meio tempo), a pessoa ainda pode
        // tentar "Reenviar código" manualmente.
      })
    }
